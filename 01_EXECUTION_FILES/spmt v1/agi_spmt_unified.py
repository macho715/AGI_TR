...
# Import agi_ssot with path resolution
import sys
from pathlib import Path
import importlib.util

# Add script directory to sys.path for module resolution
...
try:
    ...
    agi_ssot_path = script_dir / "agi_ssot.py"
    if agi_ssot_path.exists():
        spec = importlib.util.spec_from_file_location("agi_ssot", agi_ssot_path)
        agi_ssot = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(agi_ssot)
        VesselSSOT = agi_ssot.VesselSSOT
        build_agi_default_ssot = agi_ssot.build_agi_default_ssot
    else:
        raise ImportError(f"agi_ssot.py not found in {script_dir}")
